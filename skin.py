...
for i in range(len(heroid_list)):
    ...
    for skin_id in skin_ids:
        skin_url = "https://game.gtimg.cn/images/lol/act/img/skin/big" + skin_id + ".jpg"
        skin_urls.append(skin_url)
    #print(skin_urls)
    for j in range(len(skin_ids)):
        filename = "/home/yangxin/Desktop/爬虫数据/lol/" + hero_cnnames[i] + "/" + skin_names[j] + ".jpg"
        filepath = "/home/yangxin/Desktop/爬虫数据/lol/" + hero_cnnames[i] + "/"
        ...
        try:
            ...
            filename = "/home/yangxin/Desktop/爬虫数据/lol/" + hero_cnnames[0] + "/" + sname + ".jpg"
            res = requests.get(skin_urls[j])
            image = Image.open(BytesIO(res.content))
            image.save(filename)
            print('成功下载' + skin_names[j] + '.jpg')
